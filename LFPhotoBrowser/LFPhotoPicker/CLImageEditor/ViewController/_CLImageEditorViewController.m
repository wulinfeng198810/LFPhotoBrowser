...
// 过滤掉一些东西，只留下 filter, adjustment, rotate, crop
- (BOOL)filter:(CLImageToolInfo *)toolInfo
{
    NSArray *availableArr = @[
//                              [CLImageEditorTheme localizedString:@"CLFilterTool_DefaultTitle" withDefault:@"Filter"],
                              [CLImageEditorTheme localizedString:@"CLAdjustmentTool_DefaultTitle" withDefault:@"Adjustment"],
                              [CLImageEditorTheme localizedString:@"CLRotateTool_DefaultTitle" withDefault:@"Rotate"],
                              [CLImageEditorTheme localizedString:@"CLClippingTool_DefaultTitle" withDefault:@"Crop"]
                              ];
    for (NSString *title in availableArr) {
        if ([title isEqualToString:toolInfo.title]) {
            return YES;
        }
    }
    return NO;
...
{
    if(self.targetImageView==nil){
        LFImagePickerNavgationController *imgNav = (LFImagePickerNavgationController *)self.navigationController;
        [imgNav doneWithImage:_originalImage];
